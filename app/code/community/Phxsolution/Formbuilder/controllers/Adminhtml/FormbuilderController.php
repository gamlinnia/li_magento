...
class Phxsolution_Formbuilder_Adminhtml_FormbuilderController extends Mage_Adminhtml_Controller_Action
{
	...
	public function newAction()
	{
		...
		$this->_addContent(
			...
			$this->getLayout()->createBlock("formbuilder/adminhtml_formbuilder_edit_tabs")
		);
	...
	public function saveAction()
	{
		if ($data = $this->getRequest()->getPost())
		{
			...
			try
			{
				...
				{
					/* if stores contains 0, means all store viwes is selected. store 0 only */
				...
				/* if get no created time, means new record, set craeted time to now. */
				...
				/* set the id(forms_index) with setFormsIndex, ID is not in $data */
				$model->setData($data)
